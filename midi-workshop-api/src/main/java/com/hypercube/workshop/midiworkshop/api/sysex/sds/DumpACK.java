...
public record DumpACK(int channel, int packetId) implements SampleDumpStandardMessage {
    ...
    public CustomMidiEvent getRequest() {
        return MidiEventBuilder.parse("F0 7E %02X 7F %02X F7".formatted(channel, packetId))
                .getFirst();
    }
}
