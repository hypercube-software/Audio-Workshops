...
public record DumpACK(int channel, int packedId) implements SampleDumpStandardMessage {
    ...
    public static DumpACK fromBytes(byte[] payload) {
        return new DumpACK(payload[2], payload[4]);
    }

    public CustomMidiEvent getRequest() {
        return MidiEventBuilder.parse("F0 7E %02X 7F %02X F7".formatted(channel, packedId))
                .getFirst();
    }
}
